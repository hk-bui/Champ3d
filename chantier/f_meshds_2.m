% function mesh3d = f_meshds3d_2(mesh3d,varargin)
% F_MESHDS3D
%--------------------------------------------------------------------------
...
elem_type = 'hex';
get = '_all';
mesh.node = c3dobj.mesh3d.my_mesh3d.node;
mesh.elem = c3dobj.mesh3d.my_mesh3d.elem;
...
if any(strcmpi(get,{'_all','D', 'Div'}))
    ...
    % ---
    if ~all(isfield(mesh,{'face_in_elem','sign_face_in_elem'}))
        [id_face_in_elem, ori_face_in_elem, sign_face_in_elem] = ...
            f_faceinelem(elem,node,mesh.face,'elem_type',elem_type,'get','_all');
    end
    % ---
    ...
    for i = 1:nbFa_inEl
        mesh.div = mesh.div + ...
            sparse(1:nbElem,id_face_in_elem(i,:),sign_face_in_elem(i,:),nbElem,nbFace);
    end
    % ---
...
if any(strcmpi(get,{'_all', 'R', 'Rot', 'Curl'}))
    ...
    % ---
    if ~all(isfield(mesh,{'edge_in_face','sign_edge_in_face'}))
        edge_list = f_edge(mesh.face,'elem_type',f_elemtype(mesh.face,'defined_on','face'));
        [id_edge_in_face, sign_edge_in_face] = ...
            f_edgeinface(mesh.face,edge_list,'get',get);
    end
    % ---
    ...
    for k = 1:2 %---- 2 faceType
        ...
        for i = 1:nbEd_inFa{k}
            mesh.rot = mesh.rot + ...
                sparse(iface,id_edge_in_face(i,iface),sign_edge_in_face(i,iface),nbFace,nbEdge);
        end
    end
