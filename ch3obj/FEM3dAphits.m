    ...
    methods
        ...
        function assembly(obj)
            ...
            if obj.ltime.it <= 1
                ...
                % XTODO
                % APhiprev = [obj.dof{it}.A.value(id_edge_a_unknown); ...
                %             obj.dof{it}.Phi.value(id_node_phi_unknown); ...
                %             zeros(len_dphi_unknown,1)];
                Aprev = obj.dof{it-1}.A.value(1:nb_edge);
                Phiprev = obj.dof{it-1}.Phi.value(id_node_phi);
                delta_t = obj.ltime.t_array(it) - obj.ltime.t_array(it - 1);
            end
            % ---
            S11  = obj.parent_mesh.discrete.rot.' * obj.matrix.nu0nurwfwf * obj.parent_mesh.discrete.rot;
            S11  = S11 + (1/delta_t) .* obj.matrix.sigmawewe;
            S12  = obj.matrix.sigmawewe * obj.parent_mesh.discrete.grad;
            S22  = delta_t .* obj.parent_mesh.discrete.grad.' * obj.matrix.sigmawewe * obj.parent_mesh.discrete.grad;
            % --- dirichlet remove
            ...
            % ---
            prevRHS1 = (1/delta_t) .* obj.matrix.sigmawewe * Aprev;
            prevRHS2 = (obj.matrix.sigmawewe * obj.parent_mesh.discrete.grad).' * Aprev;
            % prevRHS  = [prevRHS1; prevRHS2];
            %--------------------------------------------------------------
            RHS = bsfieldRHS + pmagnetRHS + jscoilRHS + prevRHS1;
            RHS = RHS(id_edge_a_unknown,1);
            RHS = [RHS; prevRHS2(id_node_phi_unknown)];
            %--------------------------------------------------------------
            % XTODO
            % id_coil__ = {};
            % if ~isempty(obj.coil)
        ...
        function solveone(obj,args)
            ...
            if it > 1
                ...
                while improvement > tol_out && niter_out < maxniter_out
                    ...
                    obj.dof{it}.E.value = ...
                        - (1/delta_t) .* (obj.dof{it}.A.value - obj.dof{it-1}.A.value) + ...
