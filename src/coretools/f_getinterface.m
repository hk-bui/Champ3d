...
arglist = {'elem_type','of_dom3d','get'};
...
iinf = [];
for i = 1:nb2D
    iinf = [iinf find((dom_left_of_face == iDom(i,1) & dom_right_of_face == iDom(i,2)) | ...
                      (dom_right_of_face == iDom(i,1) & dom_left_of_face == iDom(i,2)))];
end
nbInt = length(iinf);
if ~isempty(iinf)
    interface = zeros(maxnbNo_inFa+3,nbInt);
    interface(1:maxnbNo_inFa,:) = face(1:maxnbNo_inFa,iinf);
    interface(maxnbNo_inFa+1,:) = dom_left_of_face(iinf);
    interface(maxnbNo_inFa+2,:) = dom_right_of_face(iinf);
