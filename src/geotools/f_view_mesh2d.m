...
%--------------------------------------------------------------------------
clear msh;
msh.Vertices = c3dobj.mesh2d.(id_mesh2d).node.';
msh.Faces = c3dobj.mesh2d.(id_mesh2d).elem(:,id_elem).';
msh.FaceColor = color;
msh.EdgeColor = 'k'; % [0.7 0.7 0.7] --> gray
patch(msh); axis equal; alpha(0.5); hold on
if ~isempty(id_elem)
    node1x = mean(c3dobj.mesh2d.(id_mesh2d).node(1, ...
                  c3dobj.mesh2d.(id_mesh2d).elem(1:4,id_elem(1))));
    node1y = mean(c3dobj.mesh2d.(id_mesh2d).node(2, ...
                  c3dobj.mesh2d.(id_mesh2d).elem(1:4,id_elem(1))));
