...
tsigPlateUp = f_make_gtensor('type','gtensor',...
    'main_value',sigPlateUp,'ort1_value',sigPlateUp,'ort2_value',sigPlateUp,...
...
mesOpt.y{2}  = {hPlateDo, 1, 'lin'};
mesOpt.y{3}  = {dgap, 3, 'lin'};
mesOpt.y{4}  = {hPlateUp-esurf, 2*msize, 'lin'};
mesOpt.y{5}  = {esurf, 1, 'lin'};
mesOpt.y{6}  = {agap, msize, 'lin'};
mesOpt.y{7}  = {hCoil, 2*msize, 'lin'};
mesOpt.y{8}  = {lPlate, 2*msize, 'log+'};
% ---
[p2d, t2d, ~, ~, ~] = f_make_mesh_xy(mesOpt);
nb_p = size(p2d,2);
nb_t = size(t2d,2);
% ---
i_elem_2d_plate_do = f_find_dom_xy(t2d,{[2 3 4]},{[2]});
i_elem_2d_plate_up = f_find_dom_xy(t2d,{[2 3 4]},{[4 5]});
i_elem_2d_esurf = f_find_dom_xy(t2d,{[2 3 4]},{[5]});
i_elem_2d_coil  = f_find_dom_xy(t2d,{[3]},{[7]});
% ---
id_dom2d_plate_do = 100;
id_dom2d_plate_up = 101;
id_dom2d_esurf = 102;
id_dom2d_coil  = 200;
t2d(5,i_elem_2d_plate_do) = id_dom2d_plate_do;
t2d(5,i_elem_2d_plate_up) = id_dom2d_plate_up;
...
design3d = f_add_dom3d(design3d,'defined_on','elem','id_dom3d','plate_do',...
                            'id_dom2d',[id_dom2d_plate_do],...
                            'id_layer',{'allLayer1','lLine','allLayer2'});
design3d = f_add_dom3d(design3d,'defined_on','elem','id_dom3d','plate_up',...
                            'id_dom2d',[id_dom2d_plate_up id_dom2d_esurf],...
                            'id_layer',{'allLayer1','lLine','allLayer2'});
...
f_view_meshquad(p2d,t2d,i_elem_2d_plate_up,f_randcolor); hold on
f_view_meshquad(p2d,t2d,i_elem_2d_plate_do,f_randcolor); hold on

%% Define physical domains

design3d = f_add_econductor(design3d,'id_dom3d','plate_up','sigma',tsigPlateUp);
...
design3d = f_add_nomesh(design3d,'id_dom3d','plate_do');
...
% (3)
design3d = f_add_bcon(design3d,'defined_on','face','id_dom3d','plate_do',...
                         'bc_type','sibc','sigma',sigPlateDo,'mur',1);
...
design3d.aphi.id_bcon_sibc  = [2 3];
%---
...
% ---
IDElem = design3d.bcon(3).id_face;
cface  = f_barrycenter(design3d.mesh.node,design3d.mesh.face(1:4,IDElem));
ynode  = max(cface(2,:));
znode  = sort(cface(3,:));
znode  = znode(round(length(znode)/2));
iface  = find(cface(2,:) == ynode & cface(3,:) == znode);
xnode  = cface(1,iface);
[xnode, ixnode] = sort(xnode);
xnode = xnode - mean(xnode);
...
figure
subplot(121)
plot(xnode,(imag(f_norm(design3d.aphi.Js(:,IDElem(ixnode))))),'b');
title('imag J');
subplot(122)
plot(xnode,(real(f_norm(design3d.aphi.Js(:,IDElem(ixnode))))),'b');
title('real J');
...
figure
f_viewthings('type','face','node',design3d.mesh.node,'face',design3d.mesh.face(:,IDElem(ixnode)),...
             'field',f_norm(abs(design3d.aphi.Js(:,IDElem(ixnode)))).');
title('|J|Inductor')
...
%              'edge',design3d.mesh.edge(1:2,design3d.nomesh(1).id_inside_edge),...
%              'type','edge');
% figure
% f_viewthings('node',design3d.mesh.node,...
%              'edge',design3d.mesh.edge(1:2,design3d.nomesh(2).id_inside_edge),...
%              'type','edge');
% % ---
% figure
% IDElem = design3d.dom3d.plate_do.id_elem;
% f_viewthings('type','elem','node',design3d.mesh.node,'elem',design3d.mesh.elem(:,IDElem),...
%              'elem_type','hex','color',f_randcolor); hold on;
% IDElem = design3d.dom3d.plate_up.id_elem;
% f_viewthings('type','elem','node',design3d.mesh.node,'elem',design3d.mesh.elem(:,IDElem),...
%              'elem_type','hex','color',f_randcolor); hold on;
